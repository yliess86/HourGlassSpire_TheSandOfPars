...
sand_chunk_propagate_dirty :: proc(sand: ^Sand_World) {
	...
	for &chunk in sand.chunks do chunk.needs_sim = chunk.dirty && chunk.active_count > 0
	...
	for &chunk in sand.chunks do chunk.dirty = false
}

// Recount active cells in all chunks (called once at init)
sand_chunk_recount :: proc(sand: ^Sand_World) {
	for &chunk in sand.chunks do chunk.active_count = 0
	...
	for &chunk in sand.chunks do if chunk.active_count > 0 do chunk.dirty = true
}
