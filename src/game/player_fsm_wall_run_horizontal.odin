...
player_fsm_wall_run_horizontal_update :: proc(ctx: ^Player, dt: f32) -> Maybe(Player_State) {
	...
	ctx.body.vel.x = PLAYER_WALL_RUN_HORIZONTAL_SPEED * ctx.abilities.wall_run_dir * combined
	ctx.body.vel.y =
	...
	if ctx.body.vel.y < -PLAYER_WALL_SLIDE_SPEED do return .Airborne
	...
	fall_speed := max(f32(0), -ctx.body.vel.y)
